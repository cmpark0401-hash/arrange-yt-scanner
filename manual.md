...
## 🛒 채널 구매·소유권 이전 가이드 (영상 4편 + 웹리서치 + 강의 노트 종합)

> **출처**: YouTube 영상 4편(P14CtfpbcYo, eZmLtGLlEQE, sIfW8U36JzU, I6jgyr4yixo) + 강의 녹취록 + 구글 공식문서 + brunch 실전 사례 + Creator Hero 가이드. **핵심 숫자 3개: 7일(소유권 승격 대기) · 32일(에센 변경 lock-in) · 30일(인수 후 침묵 권장)**

<details>
<summary><b>📖 펼쳐서 보기 — 구매 전 체크 → 7일 룰 → 30일 운영</b></summary>
...
### 5️⃣ 강의 노트 (녹취록 기반) — 구매 후 30일 액션 플랜

| 시점 | 액션 | Why |
|---|---|---|
| **1주차** | 본인 에드센스 연결 시작 (승인 ~2주 소요) | 3주차 수익부터 본인 계좌 유입 |
| **즉시** | 기존 영상 "그대로 유지" | 삭제 시 수익 정지 카더라는 사실 무근. 굳이 리스크 X |
...
## 🎯 시도남 노멀쇼츠 강의 — 채널 직구·운영 (디하클 특강 392p PDF)

> **출처**: [디하클] 시도남 노멀쇼츠 1기 특강 PDF + 녹음 6편 (학습 노트, 원문 인용 X). 핵심 슬로건: **"새로 만들지 마세요. 잘된 걸, 다시 굴리세요."** + **"0부터 만들지 않는다. 시간을 산다."**

<details>
<summary><b>📖 펼쳐서 보기 — 시도남 4단계 워크플로우 + 첫 7일 안정화</b></summary>
...
|---|---|
| 평균 3~N개월 | 즉시 운영 가능 |
| 수창 승인까지 대기 | 구독자 1,000 + 시청시간 4,000h **이미 완료** |
| 0원이지만 기회비용 큼 | 20~30만원대로 시작 가능 |

> **고수의 공통점**: "새로 만들지 않는다. **검증된 걸, 다시 굴린다.**"

### 💰 시세 — 일반 50만원 vs 검증 루트 20~30만원

- 일반 시세: 약 **50만원**
- 검증된 직거래 루트: 약 **20~30만원대**
- 가격 격차 = **정보 격차** (중간 마진 없는 직거래 + 검증된 셀러 풀 + 협상 가능 구조)

### ✅ 채널 구매 시 절대 확인할 3가지

1. **수익창출 활성 상태인가** — Studio 라이브 확인 (스샷 위조 가능)
2. **노란딱지/저작권 이력 없는가** — 콘텐츠 → 제한사항 + 저작권 페이지
3. **채널 소유권 이전 100% 가능한가** — Brand Account 상태여야 가능

### 🛡️ 안전 거래 핵심 — "싸게 사기보다 안전하게 사기"

- ✅ **에스크로(중개) 거래** 사용 (직거래 절대 X)
- ✅ **소유권 이전 완료 후 잔금** (7일 룰 활용한 단계별 입금)
...
### 🚦 구매 후 첫 7일 — "알고리즘이 정상 채널로 인식하게"

- ✅ **위험 요소 영상 X** (아이·동물·피 등 sensitive 콘텐츠 금지)
- ✅ **템플릿 유지** — 폰트·자막·결·BGM 급변 금지
- ✅ **1일 1개씩** 천천히 (폭발적 업로드 X)
...
> "허위·재사용으로 한 번 오염된 계정에 영상 다 지우고 다시 키우기는 비추 — **걱정·불안 없는 깔끔한 계정**을 가져가야 멘탈 관리에 좋음" (시도남 강의 노트)

### 💡 거래 꿀팁 (TIP 슬라이드 p.289)

- **판매자 순위 20위 안**에서 구매 (검증된 셀러 풀)
- **여러 개 묶음 구매 시 가격 조정** 요청
- **AS 미리 요구**: "5개 업로드 시 조회수 100 안 나오면 교체 요청"
- 문제 발생 시 **중재자(에스크로)에 도움 요청**

### 🌏 정규 강의 모듈 (참고 — 채널 운영 관련만)

**2주차 모듈**:
- 다른 채널 제작방법 역추적 (원본 찾기 · 템플릿 파악 · 폰트·자막 결 분석)
- 캡컷 초급(고급) — 장면 분할 · 얼빡 · 품질 보정 · 복합 클립 · 기승전결 컷 흐름
- 업로드 직접 시연 — 음악·썸네일·태그·설명문
- **애드센스 연결방법** — 수익창출 계정 + 애드센스 연결
- 운영 시트 + "10편 완주" 강제 룰
- **첫 48시간 / 10편 업로드 후 데이터 보는 법** — 무엇을 보고 다음 영상 결정

**3주차 모듈**:
- **일본 채널 셋업 시연** — 일본 채널 생성·보안·운영 셋업
- 한국 → 일본 복제 핵심 원칙
- **다채널 2~4등 전략 (합산 1등)** — 채널 1등이 아니라 **다채널 합산 1등**
  - 채널당 **100~300만원 라인** 설계
  - 한국 + 일본 = 시장 분산 = 안정성
- 항소 제작 (저작권 대응) — 노란딱지 발생 시 대응법 + 저작권 안전 원칙 3가지
- 인스타형 — "잘된 건 또 잘된다" 원칙 + 변주 기준 5가지 (톤·컷 수·자막·결말·시작점)

### 🔗 연관 섹션

- 🛒 [채널 구매·소유권 이전 가이드](#) — 7일/32일/30일 핵심 숫자, 영상 4편 + 웹리서치 종합
- 💰 [에드센스 운영 가이드](#) — 월억남 학습 노트, 본인인증·세금정보·계좌 4단계
